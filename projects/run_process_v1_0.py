...
def make_predictions_and_print(config_file):
    ...
    output_path = config_object["OUTPUT"]["output_path"]
    final_result = config_object["OUTPUT"]["final_result"]
    ...
    # Convert predictions array to a 1D array or list
    predictions_1d = predictions.flatten() if predictions.ndim > 1 else predictions.tolist()

    # Create a DataFrame for predictions
    predictions_df = pd.DataFrame({'Predictions': predictions_1d})
    
    # Save the predictions DataFrame to a new CSV file
    output_csv_path = os.path.join(final_result, 'predictions_n.csv')
    predictions_df.to_csv(output_csv_path, index=False)
    
    # Print or use the predictions as needed
    print('Nitrogen Content (dry weight) is %.3f percent'%predictions)
...
parser.add_argument("-c", "--config", help="add a configuration file you would like to process the asd data"
                                              " \n ex. py run_asd_processing.py -c spectro_config_n.conf",
                       action="store", default='spectro_config_n.conf')
...
def make_predictions_and_print(config_file):
    ...
    # Save the predictions DataFrame to a new CSV file
    output_csv_path = os.path.join(final_result, 'predictions_p.csv')
    predictions_df.to_csv(output_csv_path, index=False)
    
    # Print or use the predictions as needed
    print('Phosphorus Content (dry weight) is %.3f percent'%predictions)
...
parser = argparse.ArgumentParser(description='Argument indicating the configuration file')
parser.add_argument("-c", "--config", help="add a configuration file you would like to process the asd data"
                                              " \n ex. py run_asd_processing.py -c spectro_config_p.conf",
                       action="store", default='spectro_config_p.conf')
args = parser.parse_args()
config_file = args.config
#print('config file: ' + config_file)
...
def make_predictions_and_print(config_file):
    ...
    # Save the predictions DataFrame to a new CSV file
    output_csv_path = os.path.join(final_result, 'predictions_k.csv')
    predictions_df.to_csv(output_csv_path, index=False)
    
    # Print or use the predictions as needed
    print('Potassium Content (dry weight) is %.3f percent'%predictions)
...
parser = argparse.ArgumentParser(description='Argument indicating the configuration file')
parser.add_argument("-c", "--config", help="add a configuration file you would like to process the asd data"
                                              " \n ex. py run_asd_processing.py -c spectro_config_k.conf",
                       action="store", default='spectro_config_k.conf')
args = parser.parse_args()
config_file = args.config
#print('config file: ' + config_file)
